# Google Colab: stego-датасет → Yandex Object Storage (S3)

Одна инструкция. Копируй по ячейкам в Colab.  
Картинки будут загружаться в бакет `stegopractice` в “папку” `Stego/`:

- `Stego/<dataset>/identity/00001234.png`

Где `<dataset>` это `chatgpt|laion|coco|flickr`, а число — индекс строки (0-index) в файле промптов.  
Если объект уже существует — он пропускается (можно перезапускать чанки).

---

## Ячейка 1 — клонирование и зависимости
...
Если ранее ловил OOM — просто **Runtime → Restart runtime** и начинай с этой ячейки.

## Ячейка 2 — скачать веса (SD + CLIP)

```bash
!mkdir -p weights
# ВАЖНО: берём fp16-чекпойнт (меньше RAM/VRAM, стабильнее в Colab).
#
# Если хочешь оригинальный fp32 (7+ ГБ), используй gdown:
# !gdown "https://drive.google.com/uc?id=1ISeumzN-JrhyAacOPlh1IjAv6MKffogU" -O weights/v1-5-pruned.ckpt
#
# fp16 ckpt (~4.3 ГБ):
!wget -O weights/v1-5-pruned-emaonly.fp16.ckpt "https://huggingface.co/genai-archive/stable-diffusion-v1-5/resolve/main/v1-5-pruned-emaonly.fp16.ckpt"
```
...
## Ячейка 3 — генерация чанка и загрузка в `Stego/`

Меняешь только:
- `DATASET` (`chatgpt|laion|coco|flickr`)
- `START` (0-index)
- `COUNT` (сколько картинок)

Ключи **не вшиваются в репозиторий**. Вставь их в Colab сюда (или используй Colab Secrets).
...
DATASET = "chatgpt"   # chatgpt|laion|coco|flickr
START = 0             # 0-index
COUNT = 10            # например 500 или 1000
...
Выбери фиксированный `COUNT` (например 1000). Для \(k\)-й сессии:

- `START = k * COUNT`
- `COUNT = COUNT`

Пример при `COUNT=1000`:
- сессия 0: `START=0`
- сессия 1: `START=1000`
- сессия 2: `START=2000`
...
Одна инструкция. Копируй по ячейкам в Colab.  
Результат будет загружаться в бакет `stegopractice` в “папку” `Stego/`:

- `Stego/<dataset>/identity/00001234.png`

Где `<dataset>` это `chatgpt|laion|coco|flickr`, а число — индекс строки в файле промптов (0-index).  
Это помогает не повторяться: если объект уже есть — он пропускается.
...
И вставляешь **свои** ключи (не публикуй их).
...
# Google Colab: генерация stego-датасета и загрузка в Yandex Object Storage (S3)

Цель: запускать **в разных сессиях Colab** и собирать stego-изображения в бакет `stegopractice` в папку `Stego/`.
...
В этой ячейке **меняешь только**:
- `DATASET` (`chatgpt|laion|coco|flickr`)
- `START` (с какой строки, 0-index)
- `COUNT` (сколько строк/картинок)
...
# ====== НАСТРОЙКИ ЧАНКА (меняй это между сессиями) ======
DATASET = "chatgpt"   # chatgpt|laion|coco|flickr
START = 0             # 0-index
COUNT = 10            # например 500 или 1000

# ====== S3 (Yandex Object Storage) ======
# Вставь свои ключи (без лишних кавычек внутри строки).
AWS_ACCESS_KEY_ID = "..."
...
S3_ENDPOINT_URL = "https://storage.yandexcloud.net"
S3_BUCKET = "stegopractice"
S3_PREFIX = "Stego"   # "папка" в бакете

# Защита от ошибок вида '"ru-central1"' или '"https://..."'
def _strip_quotes(x: str) -> str:
    return x.strip().strip('"').strip("'")
